...
def plot_all_metrics_as_bar_chart():
    ...
    file = r'data\result_lesion\backbone.csv'
    df = pd.read_csv(file)
    ...
    for j in range(len(x)):
        ...
        ax2.text(x[j] + offset, params[j] + 0.1, f'{params[j]:.2f}',
                 ha='center', va='bottom', fontsize=8, color='black')
    ax2.set_ylabel('Params (M)', color='black')
    ax2.tick_params(axis='y', labelcolor='black')  # 右轴字体黑色
    ax2.set_ylim(0, 60)
    ...
    for j in range(len(x)):
        ...
        ax3.text(x[j] + offset, inference_time[j] + 0.1, f'{inference_time[j]:.2f}',
                 ha='center', va='bottom', fontsize=8, color='black')
    ax3.set_ylabel('Inference Time (ms)', color='black')
    ax3.tick_params(axis='y', labelcolor='black')  # 右轴字体黑色
    ax3.set_ylim(15, 65)
