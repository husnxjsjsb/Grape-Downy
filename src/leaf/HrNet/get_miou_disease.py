...
from nets.hrnet import HRnet as unet
from utils.utils import cvtColor, preprocess_input, resize_image, show_config

class Unet1(object):
    _defaults = {
        "model_path": r"C:\model\hrnet-pytorch-main\hrnet-pytorch-main\logs-hernet_dis\best_epoch_weights.pth",
        "num_classes": 2,
    ...
    def generate(self, onnx=False):
     self.net = unet(num_classes=self.num_classes, backbone=self.backbone, pretrained=False)
    # 明确设备选择逻辑
    ...
    def get_miou_png(self, image):
        ...
        with torch.no_grad(): 
            ...
            # 将预测结果转换为指定颜色 
            color_pr = np.zeros((pr.shape[0],  pr.shape[1],  3), dtype=np.uint8) 
            for i in range(len(self.colors)): 
                color_pr[(pr == i)] = self.colors[i] 
            
            return Image.fromarray(color_pr) 
 

if __name__ == "__main__":
    miou_mode = 0
    num_classes = 2
    name_classes = ["_background_", "disease"]
