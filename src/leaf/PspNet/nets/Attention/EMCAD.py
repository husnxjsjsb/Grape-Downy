...
'''来自CVPR2024EMCAD: Efficient Multi-scale Convolutional Attention Decoding for MedicalImage Segmentation
EMCAM多尺度卷积注意力特征融合模块：
本文引入了一个有效的多尺度卷积注意模块来细化特征映射。
MSCAM由一个通道注意块（CAB（·）），一个空间注意块（SAB（·））
来捕获局部上下文信息，以及一个高效的多尺度卷积块（MSCB（.））
来增强保留上下文关系的特征映射。
'''
...
class EMCAM(nn.Module):
    def __init__(self, in_channels=64, out_channels=64, kernel_sizes=[1, 3, 5], expansion_factor=6, dw_parallel=True,
