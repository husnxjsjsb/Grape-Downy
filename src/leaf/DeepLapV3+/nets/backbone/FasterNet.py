...
class FasterNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 patch_norm=True,
                 feature_dim=1280,
                 drop_path_rate=0,#fasternet1=0.02
    ...
    def forward_cls(self, x):
        ...
        x = self.stages(x)  # Pass through all stages
        x = self.avgpool_pre_head(x)  # B C 1 1
        x = torch.flatten(x, 1)  # Flatten the features to (B, C)
        x = self.head(x)  # Linear head for classification
        return x  # Return logits for classification
    
    def forward_det(self, x: Tensor) -> dict:
        # 返回特征图的字典
        x = self.patch_embed(x)
        outs = {}
        for idx, stage in enumerate(self.stages):
            ...
            if self.fork_feat and idx in self.out_indices:
                ...
                x_out = norm_layer(x)
                outs[f'output_stage_{idx}'] = x_out
        
        return outs
    def forward(self, x: Tensor):
        if self.fork_feat:
            return self.forward_det(x)  # 如果需要输出多个阶段的特征图
        else:
