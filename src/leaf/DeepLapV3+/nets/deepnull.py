...
class DeepLab(nn.Module):
    def __init__(self, num_classes, backbone="mobilenetv3", pretrained=True, downsample_factor=16, attention_type=None):
        ...
        if backbone == "xception":
            ...
            self.backbone = EfficientNetBackbone(model_name='efficientnet-b2', pretrained=pretrained,
                                                 downsample_factor=downsample_factor)
            in_channels = 320 if downsample_factor == 16 else 120  # EfficientNet-B2中 reduction_5 和 reduction_4 的输出通道数
            low_level_channels = 32 if downsample_factor == 16 else 32  # EfficientNet-B2中 reduction_1 和 reduction_2 的输出通道数
