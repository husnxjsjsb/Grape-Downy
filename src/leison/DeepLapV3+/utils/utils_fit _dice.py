...
class FocalDiceLoss(nn.Module):
    ...
    def forward(self, outputs, targets, weights, num_classes):
        ...
        total_loss = focal_loss.mean() + dice_loss.mean()
...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, \
    fp16, scaler, save_period, save_dir, local_rank=0, use_focal_dice_loss=False):
    total_loss      = 0
    total_f_score   = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            #   计算损失
            #----------------------#
            if use_focal_dice_loss:
                focal_dice_loss = FocalDiceLoss()
                loss = focal_dice_loss(outputs, pngs, weights, num_classes)
            else:
                if focal_loss:
    ...
    if local_rank == 0:
        ...
        if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
            print('Save best model to Deeplapv3.pth.pth')
            torch.save(model.state_dict(), os.path.join(save_dir, "Deeplapv3.pth"))
