...
# from nets.unet_mobilenetv3 import Unet as unet
# from nets.unet_mobilenetv3 import Unet  as unet
from nets.unet_mobile import Unet as unet
from utils.utils import cvtColor, preprocess_input, resize_image, show_config
class Unet1(object):
    _defaults = {
        "model_path": r"C:\model\unet-pytorch-main\wuunet4\unet_eff_rep_ds.pth",
        "num_classes": 3,
...
if __name__ == "__main__":
    ...
    if miou_mode == 0 or miou_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path = os.path.join(VOCdevkit_path, "VOC2007/JPEGImagesdeep-mv3//" + image_id + ".jpg")
    ...
    if miou_mode == 0 or miou_mode == 2:
        ...
        csv_file_path = os.path.join(miou_out_path, 'unet_results.csv')

        if not os.path.exists(csv_file_path):
            with open(csv_file_path, mode='w', newline='') as file:
                writer = csv.writer(file)
                # 写入表头
                writer.writerow(["Model", "mPA", *[f"IoU ({name})" for name in name_classes], "mIoU", "Accuracy", "Params (M)", "Inference Time (ms)"])
