...
from nets.nets.mobilenetv2 import mobilenetv2
# from nets.mobilenetv3 import mobilenet_v3
from nets.nets.resnet import resnet50
from efficientnet_pytorch import EfficientNet
# from nets.FasterNet import FasterNet
from nets.nets.Starnet import starnet_s1
...
#         self.model = FasterNet(fork_feat=True, pretrained=pretrained)
...
#             x_aux = features[2]  # 选择第3个特征层作为辅助分支输入
#             x = features[3]      # 选择第4个特征层作为主干分支输入
#         elif self.downsample_factor == 16:
#             x_aux = features[1]  # 选择第2个特征层作为辅助分支输入
#             x = features[3]      # 选择第4个特征层作为主干分支输入
#         else:
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, downsample_factor, backbone="resnet50", pretrained=True, aux_branch=True):
        ...
        if backbone == "resnet50":
            ...
            aux_channel = 48  # 修改为 MobileNetV3 低层特征的通道数
            out_channel = 320  # 修改为 MobileNetV3 高层特征的通道数
