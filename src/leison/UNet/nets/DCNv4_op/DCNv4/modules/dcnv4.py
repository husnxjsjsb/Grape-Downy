...
class DCNv4(nn.Module):
    ...
    def _reset_parameters(self):
        nn.init.constant_(self.offset_mask.weight.data, 0.)
        nn.init.constant_(self.offset_mask.bias.data, 0.)
    ...
    def forward(self, input, shape=None):
        ...
        if not self.without_pointwise:
            x = self.value_proj(x)
            print("After value_proj:", x.shape)
        ...
        if self.dw_kernel_size is not None:
            ...
            print("Offset mask input after depthwise conv:", offset_mask_input.shape)
        else:
            offset_mask_input = input

        offset_mask = self.offset_mask(offset_mask_input).reshape(N, H, W, -1)
        print("Offset mask shape:", offset_mask.shape)

        x_proj = x

        # Placeholder for DCNv4Function. You should replace this with the actual function if implemented.
        ...
        if not self.without_pointwise:
            ...
            print("After output_proj:", x.shape)
