...
def CE_Loss(inputs, target, cls_weights, num_classes=2):
    ...
    return CE_loss
def Combined_Loss(inputs, target, cls_weights, num_classes=21, alpha=0.5, gamma=2, beta=1, smooth=1e-5, focal_weight=0.5, dice_weight=0.5):
    """
    结合 Focal Loss 和 Dice Loss 的新损失函数。

    参数:
        inputs: 模型输出的预测值 (N, C, H, W)
        target: 真实标签 (N, H, W) 或 (N, H, W, C)
        cls_weights: 类别权重
        num_classes: 类别数量
        alpha: Focal Loss 的 alpha 参数
        gamma: Focal Loss 的 gamma 参数
        beta: Dice Loss 的 beta 参数
        smooth: Dice Loss 的平滑参数
    ...
    # 加权求和
    combined_loss = focal_weight * focal_loss + dice_weight * dice_loss
