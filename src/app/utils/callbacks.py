    ...
    def get_miou_png(self, image):
        ...
        with torch.no_grad():
            ...
            if pr is None or pr.size == 0:
                print(f"Warning: Predicted image is empty for image of size ({orininal_w}, {orininal_h})")
                return None
    ...
    def on_epoch_end(self, epoch, model_eval):
     if epoch % self.period == 0 and self.eval_flag:
        ...
        for image_id in tqdm(self.image_ids):
            ...
            if image is None:
                print(f"Warning: Image {image_id} could not be processed. Skipping.")
                continue  # 如果图像为空，则跳过当前图像
